...
def group_files_without_renaming(base_path, new_base_path, group_size):
    #for split in ['train', 'valid', 'test']:
    for split in ['train', 'test']:
        ...
        for i in range(num_groups):
            ...
            for j in range(group_size):
                img_index = i * group_size + j
                original_img_name = images[img_index]
                original_label_name = labels[img_index]

                shutil.copy(os.path.join(image_dir, original_img_name),
                            os.path.join(new_image_dir, group_name, original_img_name))
                shutil.copy(os.path.join(label_dir, original_label_name),
                            os.path.join(new_label_dir, group_name, original_label_name))
        ...
        if rest_count > 0:
            ...
            for i in range(rest_count):
                ...
                shutil.copy(os.path.join(image_dir, original_img_name), os.path.join(rest_image_dir, original_img_name))
                shutil.copy(os.path.join(label_dir, original_label_name),
                            os.path.join(rest_label_dir, original_label_name))

if __name__ == "__main__":
    base_path = "../image_data/tobacco_split_cleaned_iou0"
    new_base_path = "../image_data/tobacco_grouped_split_cleaned_iou0"
    group_size = 10

    create_grouped_structure(base_path, new_base_path, group_size)
    group_files_without_renaming(base_path, new_base_path, group_size)
