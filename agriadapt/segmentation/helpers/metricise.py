...
from agriadapt.segmentation.helpers.masking import get_binary_masks_infest
# from adaptation.inference import AdaptiveWidth
# from adaptation.new_inference import NEW_AdaptiveWidth
...
class Metricise:
    ...
    #         self._calculate_adaptive_NEW(adaptive_knns, name, loader, model) #model_name,
    #
    # def evaluate_nonSNN(
    #     self,
    #     model,
    ...
    # def _calculate_adaptive_NEW(self, name, model_name, loader, model):
    #     width_selector = NEW_AdaptiveWidth(name)
    #     key = f"{model_name}/adapt"
    #     for X, y in loader:
    #         image = ImageImporter("geok").tensor_to_image(X)[0]
    #         X, y = X.to(self.device), y.to(self.device)
    #         width = width_selector._get_image_width(image)
    #         model.set_width(width)
    #         y_pred = model.forward(X)
    #         self.calculate_metrics(y, y_pred, key)
